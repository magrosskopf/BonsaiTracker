...
function createSubEntry(overrides: Partial<SubEntryDto> = {}): SubEntryDto {
  return {
    id: 5,
    bonsaiId: 1,
...
function createBonsaiDetail(overrides: Partial<BonsaiDetail> = {}): BonsaiDetail {
  return {
    id: 1,
    name: "Itoigawa",
    species: "Unbekannt",
    carePlanSpeciesId: null,
    carePlanActive: false,
    carePlanVersion: null,
    carePlanActivatedAt: null,
    carePlanReplacedAt: null,
    latinName: null,
    location: "Unbekannt",
...
test("bonsai timeline images keep initial bonsai photos and sort them with createdAt fallback", () => {
  const bonsai = createBonsaiDetail();
...
test("bonsai timeline helper returns an empty list without a bonsai", () => {
  assert.deepEqual(collectBonsaiTimelineImages(null), []);
});
