...
Die Startseite wird auf drei klare Auth-Wege reduziert: Google-Fortfahren, E-Mail-Anmeldung und E-Mail-Registrierung. Waitlist-UI, Web-Signup-Gating, Waitlist-APIs, v1-Aliase, Operator-Skripte und aktive Betriebsdokumentation werden aus der Anwendungsschicht entfernt. Bestehende Supabase Auth-Flows fuer Google, E-Mail/Passwort, Passwort-Reset und der env-gesteuerte Magic-Link-Fallback bleiben erhalten.
...
2. `pages/index.tsx` vereinfachen: Waitlist-Import und Renderpfade entfernen, Copy anpassen, Google-Label zu `Mit Google fortfahren` aendern, Signup direkt ueber Supabase ausfuehren, Magic-Link-Fallback ohne Precheck lassen.
...
- `getAuthModeTitle`, Labels und Auth-Helper bleiben exportiert, damit die bestehenden Source-Tests stabile UI-Kontrakte pruefen koennen.
...
- `tests/home-login.test.ts` prueft Labels, Modus-Titel, Fehlertexte, direkte Supabase-Aufrufe und Abwesenheit von Waitlist-/Precheck-Code in `pages/index.tsx`.
