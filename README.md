...
- Set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` for browser auth.
- Keep server-only Supabase access on `SUPABASE_SECRET_KEY` and `SUPABASE_STORAGE_BUCKET`.
